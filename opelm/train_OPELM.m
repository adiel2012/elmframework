function [model,TrainingTime]=train_OPELM(data,kernel,maxneur,problem,normal,KM)
...
if problem=='c'
    [temp_max pos_max] = max(yh');
    temp_labels = ones(size(yh))*(-1);

    for i = 1:N
       temp_labels(i,pos_max(i)) = 1; 
    end
...
% JAVI
% TODO: not sure if time meassurement should be finished before
%end_time = cputime;
%TrainingTime = end_time - start_train_time;
