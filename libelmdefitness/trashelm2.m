...
function [TrainingTime, ConfusionMatrixTrain, ConfusionMatrixTest, CCRTrain, MSTrain, CCRTest, MSTest...
    ...
    trashelm(train_data, test_data, Elm_Type, NumberofHiddenNeurons, ActivationFunction,  wMin, wMax)
...
NumberofHiddenNeurons = size(P,1); % floor(size(P,2)/20);  %  max(floor(sqrt(size(P,2))), size(P,1));
NumberofHiddenNeuronsFinal = NumberofHiddenNeurons;
P = P';
opts = statset('MaxIter',20);
[IDX, C, SUMD, D] = kmeans(P,NumberofHiddenNeurons,'Options',opts);    % NumberofHiddenNeurons
