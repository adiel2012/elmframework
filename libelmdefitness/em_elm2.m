...
function [TrainingTime, ConfusionMatrixTrain, ConfusionMatrixTest, CCRTrain, MSTrain, CCRTest, MSTest...
    	NumberofInputNeurons,NumberofHiddenNeurons,NumberofOutputNeurons,InputWeight,OutputWeight,pstar_train] = ...
    em_elm2(train_data, test_data, Elm_Type, NumberofHiddenNeurons, ActivationFunction,  wMin, wMax)
...
while i <= NumberofHiddenNeurons && error > errorthresold 
    ...
    if(i==2)
       ...
       T=rand(numpatterns,numoutputneurons); 
       %B1=pinv(H')*T 
       d =  H(i,:);   %rand(numpatterns,1);    
       %H2=[H' d]'; 
       %HI = (inv(H'*H)*H')'   % pinv(H'); 
    ...
    Btemp= Hi*T;
    error = sum(sum(abs( H*Btemp -T ))) / (num_patterns*numoutputneurons);
    NumberofHiddenNeuronsFinal=i;
    i = i+1;
end

H=HTemporal;
InputWeight=InputWeight(NumberofHiddenNeuronsFinal,:);
